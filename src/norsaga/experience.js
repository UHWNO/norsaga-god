...
export function initNorSagaExperience({ styleManager, dataManager, viewer, signal, documentRef = document }) {
  ...
  for (const region of REGIONS) select.append(new Option(region.label, region.id));
  ...
  for (const layer of MARITIME_LAYERS) {
    ...
    words.append(make('strong', '', layer.label), make('small', '', layer.note));
    label.append(input, words);
    layerList.append(label);
    bind(input, 'change', async () => {
      input.disabled = true;
      try {
        const accepted = await dataManager.setEnabled(layer.id, input.checked, { origin: 'user' });
        if (!disposed) report(accepted === false ? 'Feed request could not complete. Check Data Layers.' : 'Layer selection updated. Check Data Layers for source status.');
      } catch {
        if (!disposed) report('Feed request failed. Check Data Layers for details.');
      } finally {
        if (!disposed) { input.disabled = false; syncLayers(); }
      }
    });
  }
  layerList.append(make('p', 'norsaga-fineprint', 'Use Data Layers for all sources and their availability. An enabled layer is not proof of live coverage.'));
  ...
  header.append(make('span', 'norsaga-eyebrow', 'NORSAGA / MARITIME INTELLIGENCE'));
  ...
  dialog.append(header, title, make('p', 'norsaga-intro', 'Maritime technology. Secure by design.'));
  const choices = make('div', 'norsaga-missions');
  for (const mission of MISSIONS) {
    const choice = button('', () => launch(mission.id), choices, 'norsaga-mission');
    choice.dataset.norsagaMission = mission.id;
    choice.append(make('strong', '', mission.label), make('span', '', mission.description));
  }
  ...
  for (const [id, label] of [['contacts', 'Live contacts'], ['space-missions', 'Space missions'], ['environmental', 'Environmental']])
  ...
  suppressLabel.append(suppress, doc.createTextNode('Do not show this at startup'));
  const dialogStatus = make('p', 'norsaga-status');
  dialogStatus.setAttribute('role', 'status');
  dialog.append(suppressLabel, dialogStatus, make('p', 'norsaga-fineprint', 'Situational intelligence only. Not for navigation or safety-critical decisions. AIS requires a configured provider; availability and coverage vary.'));
  ...
  about.append(aboutTitle, make('p', '', 'NorSaga Maritime Intelligence AS. Built on God’s Eye View by Bilawal Sidhu, using CesiumJS.'));
  about.append(make('p', '', 'The source-code license and third-party data licenses are separate. This evaluation branch is not cleared for commercial deployment. Required map and data-provider credits remain available on the globe.'));
  const links = make('div', 'norsaga-about-links');
  for (const [path, label] of [['LICENSE', 'Software license'], ['THIRD_PARTY_NOTICES.md', 'Third-party notices'], ['docs/NORSAGA_DATA_COMPLIANCE.md', 'Data-source review']]) {
  ...
  function syncLayers() {
    for (const [id, input] of layerInputs) input.checked = dataManager.isEnabled(id);
  }
  async function launch(id, legacy = false) {
    ...
    const controls = [...choices.querySelectorAll('button'), ...advancedChoices.querySelectorAll('button')];
    ...
    try {
      const result = legacy
        ? await runFirstRunChoice(id, {
          ...
          setLayerEnabled: (layer) => dataManager.setEnabled(layer, true, { origin: 'user' }),
      ...
      if (!result.ok) {
        report('The view could not start. Leave cockpit mode or check Data Layers, then retry.');
        return;
      }
      const incomplete = result.layers?.some((layer) => layer.outcome !== 'requested');
      report(incomplete ? 'View opened. Some feed requests are pending or unavailable; check Data Layers.' : 'View selected. Data Layers shows feed availability and source status.');
      dialog.close();
    } catch (error) {
      if (!disposed && !request.signal.aborted) report('The view could not start. Try Global overview or check Data Layers.');
    } finally {
      if (!disposed) {
        busy = false;
  ...
  bind(select, 'change', () => {
    ...
    try {
      ...
      report(ok === false ? 'Leave cockpit mode before changing the operating area.' : 'Region view selected. This changes the camera, not data coverage.');
    } catch { report('This region could not open.'); }
    select.value = '';
  });
  bind(dialog, 'close', () => {
    activeRequest?.abort();
    if (disposed) return;
    rememberFirstRunSessionDismissed();
    if (suppress.checked && !setFirstRunSuppressed(true)) report('The view preference could not be saved in this browser.');
  });
  const unsubscribe = dataManager.subscribeActivity?.(() => { if (!disposed) syncLayers(); });
  syncLayers();
  if (shouldShowFirstRun({ hasShareState: styleManager.hasShareState })) dialog.showModal();
  function destroy() {
    if (disposed) return;
    disposed = true;
    activeRequest?.abort();
